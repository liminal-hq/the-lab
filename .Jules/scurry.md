...
- When implementing area-of-effect mechanics like scaring birds or shattering projectiles, use `Math.hypot()` for a true 2D radius calculation instead of just `Math.abs(x)` to avoid creating infinite vertical columns of effect.
