...
- **Squeak Mechanic Area-of-Effect**: When adding secondary interactive effects to a single input (like Squeak scaring birds *and* shattering projectiles), ensure the one-time input consumption flag (`state.input.squeakPressed = false`) is executed *after* all dependent logic blocks have processed the input. Early consumption prevents secondary mechanics from triggering.
