...
## Squeak Shattering Mechanic
- **Learning**: Expanding a defensive mechanic (Squeak scaring birds) to also affect their offspring/projectiles (shattering falling turds) increases the utility of the input without adding new buttons. Using `Math.hypot` for true 2D radius distance checking instead of `Math.abs` prevents infinite vertical/horizontal bands of effect.
