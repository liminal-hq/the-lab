...
function generateLevel() {
    ...
    let seed = seedParam ? Array.from(seedParam).reduce((s, c) => Math.imul(31, s) + c.charCodeAt(0) | 0, 0) : Math.floor(Math.random() * 0xFFFFFFFF);
...
function generateSurface() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 100 + seededRandom() * 200;
        const h = 100 + seededRandom() * (canvas.height - 200);
        // Coloured like the gloom of night, with district flavour.
        const hue = hueBase + (seededRandom() * 40 - 20);
        ...
        //     ( \/ )  <-- "Watch your step!"
        //      \  /
        const gap = seededRandom() * (gapMax - gapMin) + gapMin; // District-based spacing

        // Collectibles are mutually exclusive with standard obstacles to avoid traps
        if (seededRandom() < obsChance) {
    ...
    for(let i=0; i<5; i++) {
        state.birds.push({
            x: seededRandom() * 2000,
            y: seededRandom() * (canvas.height/2),
            speed: 1 + seededRandom() * 2,
            vy: 0
        });
    }
}
...
function generateThirdLevel() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 140 + seededRandom() * 180;
        const h = 120 + seededRandom() * (canvas.height - 220);
        const hue = district.hueBase + (seededRandom() * 30 - 15);
        const gap = seededRandom() * (district.gapMax - district.gapMin) + district.gapMin;

        state.buildings.push({ x, w, h, color: `hsl(${hue}, 25%, 28%)` });

        if (seededRandom() < district.obsChance) {
