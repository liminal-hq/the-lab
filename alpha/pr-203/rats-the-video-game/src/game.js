...
function initPRNG(level) {
    const urlParams = new URLSearchParams(window.location.search);
    const seed = urlParams.get('seed');
    if (seed) {
        const seedStr = seed + level;
        const seedHash = cyrb128(seedStr);
        genRandom = mulberry32(seedHash[0]);
    } else {
...
function generateSurface() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 100 + genRandom() * 200;
        const h = 100 + genRandom() * (canvas.height - 200);
        // Coloured like the gloom of night, with district flavour.
        const hue = hueBase + (genRandom() * 40 - 20);
        ...
        //     ( \/ )  <-- "Watch your step!"
        //      \  /
        const gap = genRandom() * (gapMax - gapMin) + gapMin; // District-based spacing

        // Collectibles are mutually exclusive with standard obstacles to avoid traps
        if (genRandom() < obsChance) {
            let rand = genRandom();
            let type = '';
            let objW = 30;
            let objH = 30;
...
function generateThirdLevel() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 140 + genRandom() * 180;
        const h = 120 + genRandom() * (canvas.height - 220);
        const hue = district.hueBase + (genRandom() * 30 - 15);
        const gap = genRandom() * (district.gapMax - district.gapMin) + district.gapMin;

        state.buildings.push({ x, w, h, color: `hsl(${hue}, 25%, 28%)` });

        if (genRandom() < district.obsChance) {
