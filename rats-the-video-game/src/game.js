...
let levelRand = mulberry32(levelSeed);

function generateLevel() {
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('seed')) {
        levelSeed = parseInt(urlParams.get('seed'), 10);
    } else {
        levelSeed = Math.floor(Math.random() * 1000000);
    }
    levelRand = mulberry32(levelSeed);
...
function generateSurface() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 100 + levelRand() * 200;
        const h = 100 + levelRand() * (canvas.height - 200);
        // Coloured like the gloom of night, with district flavour.
        const hue = hueBase + (levelRand() * 40 - 20);
        ...
        //     ( \/ )  <-- "Watch your step!"
        //      \  /
        const gap = levelRand() * (gapMax - gapMin) + gapMin; // District-based spacing

        // Collectibles are mutually exclusive with standard obstacles to avoid traps
        if (levelRand() < obsChance) {
            let rand = levelRand();
            let type = '';
            let objW = 30;
            let objH = 30;
            ...
            //      (> <)
            if (levelRand() < 0.25) {
                 const pizzaX = x + w + gap / 2 + (levelRand() * 40 - 20);
                 ...
                 //      c[_]
                 const coffeeX = x + w + gap / 2 + (levelRand() * 40 - 20);
                 state.obstacles.push({ x: coffeeX, w: 20, h: 25, type: 'COFFEE' });
            } else if (levelRand() < 0.10) {
                 // CHEESE! (The high-value prize)
                 const cheeseX = x + w + gap / 2 + (levelRand() * 40 - 20);
...
function generateThirdLevel() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 140 + levelRand() * 180;
        const h = 120 + levelRand() * (canvas.height - 220);
        const hue = district.hueBase + (levelRand() * 30 - 15);
        const gap = levelRand() * (district.gapMax - district.gapMin) + district.gapMin;

        state.buildings.push({ x, w, h, color: `hsl(${hue}, 25%, 28%)` });

        if (levelRand() < district.obsChance) {
