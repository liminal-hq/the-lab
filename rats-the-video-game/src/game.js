...
function generateLevel() {
    const seed = getSeed();
    window.seededRandom = mulberry32(seed);
...
function generateSurface() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 100 + window.seededRandom() * 200;
        const h = 100 + window.seededRandom() * (canvas.height - 200);
        // Coloured like the gloom of night, with district flavour.
        const hue = hueBase + (window.seededRandom() * 40 - 20);
        ...
        //     ( \/ )  <-- "Watch your step!"
        //      \  /
        const gap = window.seededRandom() * (gapMax - gapMin) + gapMin; // District-based spacing

        // Collectibles are mutually exclusive with standard obstacles to avoid traps
        if (window.seededRandom() < obsChance) {
            let rand = window.seededRandom();
            let type = '';
            let objW = 30;
            let objH = 30;
            ...
            //      (> <)
            if (window.seededRandom() < 0.25) {
                 const pizzaX = x + w + gap / 2 + (window.seededRandom() * 40 - 20);
                 // Floating slightly above ground logically (h=40)
                 state.obstacles.push({ x: pizzaX, w: 30, h: 40, type: 'PIZZA' });
            } else if (window.seededRandom() < 0.15) {
                 // Coffee! (The fuel of the developer... and now the rat)
                 //      c[_]
                 const coffeeX = x + w + gap / 2 + (window.seededRandom() * 40 - 20);
                 state.obstacles.push({ x: coffeeX, w: 20, h: 25, type: 'COFFEE' });
            } else if (window.seededRandom() < 0.10) {
                 // CHEESE! (The high-value prize)
                 const cheeseX = x + w + gap / 2 + (window.seededRandom() * 40 - 20);
...
function chooseThirdLevelObstacle(district, gap) {
    const roll = window.seededRandom();
...
function generateThirdLevel() {
    ...
    for (let i = 0; i < state.totalCycles; i++) {
        ...
        const w = 140 + window.seededRandom() * 180;
        const h = 120 + window.seededRandom() * (canvas.height - 220);
        const hue = district.hueBase + (window.seededRandom() * 30 - 15);
        const gap = window.seededRandom() * (district.gapMax - district.gapMin) + district.gapMin;

        state.buildings.push({ x, w, h, color: `hsl(${hue}, 25%, 28%)` });

        if (window.seededRandom() < district.obsChance) {
