...
function update() {
    ...
    } else if (state.input.jumpPressed && state.rat.canDoubleJump && !state.rat.grounded) {
        // Double Jump!
        //      (\ /)
        //      (o.o)  <-- "I believe I can fly!"
        //      (> <)
        state.rat.vy = JUMP_FORCE * 0.8;
        state.rat.canDoubleJump = false;
        spawnParticles(state.rat.x, state.rat.y - 10, '#FFF', 10); // Cloud step
        audio.playTone(880, 0.1, 15); // Higher pitch squeak
    }

    // Clear jumpPressed frame flag
    state.input.jumpPressed = false;
