...
function update() {
    ...
    if (state.input.squeakPressed) {
        if (audio && audio.playHappySqueak) audio.playHappySqueak();
        ...
        state.birds.forEach(bird => {
            if (Math.abs(bird.x - state.rat.x) < 400 && (!bird.vy || bird.vy === 0)) {
                bird.vy = -10; // Negative vy moves upwards on screen (consistent with original second block)
            }
        });

        // Shatter falling turds within 150 units
